...
class Master_model extends CI_Model {
    ...
    public function getFormJurusan()
    {
        $this->db->select('id_jurusan, nama_jurusan');
        $this->db->from('kelas');
        $this->db->join('jurusan', 'jurusan_id=id_jurusan');
    ...
    {
        $this->datatables->select('a.id_dosen,a.nip, a.nama_dosen, a.email, a.matkul_id, b.nama_kelas, (SELECT COUNT(id) FROM users WHERE username = a.nip OR email = a.email) AS ada');
        $this->datatables->from('dosen a');
        $this->datatables->join('kelas b', 'a.matkul_id=b.id_kelas');
        return $this->datatables->generate();
    }
    ...
    {
        $this->datatables->select('id_matkul, nama_matkul, nama_nilai');
        $this->datatables->from('matkul');
        $this->datatables->join('m_nilai', 'id_m=jenis_nilai');
        return $this->datatables->generate();
    }
    ...
    {
        return $this->db->get('m_nilai')->result();
    }
