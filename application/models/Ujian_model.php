...
class Ujian_model extends CI_Model {
    ...
    public function getStatusSlesai($id,$idm)
    {
    ...
    public function getUjianByMatkul($id)
    {
    ...
    public function HslUjian($id, $mhs)
    {
      ...
      //ROUND(RAND() * ( SELECT MAX(id_soal) FROM tb_soal)) AS id_soal,
    ...
    public function ambilSoalPoli($pc_urut_soal1, $pc_urut_soal_arr)
    {
        $this->db->select("*, {$pc_urut_soal1} AS jawaban");
        $this->db->from('tb_soal_poli');
        $this->db->where('id_soal', $pc_urut_soal_arr);
        return $this->db->get()->row();
    }
