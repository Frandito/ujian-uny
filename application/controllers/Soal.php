...
class Soal extends CI_Controller {
    ...
    public function detail($id)
    {
		...
		public function detailpoli($id)
		{
				...
				$data = [
						...
						'judul'	    => 'Soal',
						'subjudul'  => 'Detail Soal',
						'soal'      => $this->soal->getSoalByIdPoli($id),
						'jenis'     => '2'
				];
    ...
    public function add()
	{
        $user = $this->ion_auth->user()->row();
				...
				$data = [
					...
					'judul'	    => 'Soal',
      		'subjudul'  => 'Buat Soal',
					'nama_tes'  => $this->soal->getDetailTes($id_tes)
        ];

        if($this->ion_auth->is_admin()){
            //Jika admin maka tampilkan semua matkul
            $data['dosen'] = $this->soal->getAllDosen($id_tes);
        }else{
            //Jika bukan maka matkul dipilih otomatis sesuai matkul dosen
    ...
    public function edit($id)
	{
      ...
      'subjudul'  => 'Edit Soal',
      'soal'      => $this->soal->getSoalById($id),
			'id_tes'		=> $this->soal->getNilaiByIdSoal($id)
        ];
	...
	public function editpoli($id)
{
	  ...
	  public function data($id=null, $dosen=null)
	{
		...
		public function data1($id=null, $dosen=null)
	{
		$this->output_json($this->soal->getDataSoalPoli($id, $dosen), false);
		}
    ...
    public function validasi()
    {
        ...
        $this->form_validation->set_rules('bobot', 'Bobot Soal');
    }
        ...
        $this->file_config();
            ...
            foreach ($abjad as $abj) {
                $data['opsi_'.$abj]    = $this->input->post('jawaban_'.$abj, true);
						...
						// Inputan Bobot
						foreach ($abjad as $abj) {
								$data['bobot_'.$abj]   = $this->input->post('nilai_'.$abj, true);
            }

            $i = 0;
            foreach ($_FILES as $key => $val) {
                $img_src = FCPATH.'uploads/bank_soal/';
                $getsoal = $this->soal->getSoalByIdPoli($this->input->post('id_soal', true));
