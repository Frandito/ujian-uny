...
class Matkul extends CI_Controller
{
	...
	public function add()
	{
		$data = [
			'user' 		=> $this->ion_auth->user()->row(),
			'judul'		=> 'Tambah Tes',
			'subjudul'	=> 'Tambah Data Tes',
			'banyak'	=> $this->input->post('banyak', true),
			'metode'	=> $this->master->getAllMetode()
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function save()
	{
		...
		for ($i = 1; $i <= $rows; $i++) {
			...
			$jenis_nilai = 'jenis_nilai[' . $i . ']';
			$this->form_validation->set_rules($nama_matkul, 'Mata Kuliah', 'required');
			$this->form_validation->set_rules($jenis_nilai, 'Metode Penilaian', 'required');
			$this->form_validation->set_message('required', '{field} Wajib diisi');

			if ($this->form_validation->run() === FALSE) {
				$error[] = [
					$nama_matkul => form_error($nama_matkul),
					$jenis_nilai => form_error($jenis_nilai),
				];
				$status = FALSE;
			} else {
				if ($mode == 'add') {
