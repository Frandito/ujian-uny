...
class Ujian extends CI_Controller {
	...
	public function token($id)
	{
		$this->akses_mahasiswa();
		$user = $this->ion_auth->user()->row();
		$m = $this->ujian->getIdMahasiswa($user->username);
		$idm = $m->id_mahasiswa;
		//$status = $this->ujian->getStatusSlesai($id,$idm);
		...
		$data = [
			...
			'mhs' 		=> $this->ujian->getIdMahasiswa($user->username),
			'ujian'		=> $this->ujian->getUjianById($id),
			'peraturan'		=> $this->ujian->getPeraturan(),
	...
	public function index()
	{
		...
		if ($soal_urut_ok > 0) {
			foreach ($soal_urut_ok as $s) {
				...
				$html .= '<table border="1px" bordercolor="#D1D3D4"><tr><td><div class="text-center"><div class="center">'.tampil_media($path.$s->file).'</div></div><div style="margin-left: 10px !important; margin-right: 10px !important; margin-top: 10px !important; margin-bottom: 10px !important;">'.$s->soal.'</div></td></tr></table><div class="funkyradio">';
				for ($j = 0; $j < $this->config->item('jml_opsi'); $j++) {
					...
					$html .= '<div class="funkyradio-success" onclick="return simpan_sementara();">
						<input type="radio" id="opsi_'.strtolower($arr_opsi[$j]).'_'.$s->id_soal.'" name="opsi_'.$no.'" value="'.strtoupper($arr_opsi[$j]).'" '.$checked.'> <label for="opsi_'.strtolower($arr_opsi[$j]).'_'.$s->id_soal.'"><div class="huruf_opsi">'.ucfirst($arr_opsi[$j]).'</div> <p>'.$pilihan_opsi.'</p><div class="w-25">'.$tampil_media_opsi.'</div></label></div>';
				}
				$html .= '</div></div>';
				$no++;
			}
		...
		$data = [
			...
			'html' 		=> $html,
			'id_tes'	=> $id_tes,
			'ujin'				=> $this->ujian->getUjianById($id)
		];
		$this->load->view('_templates/topnav/_header.php', $data);
	...
	public function simpan_akhir()
	{
		...
		foreach ($pc_jawaban as $jwb) {
			...
			$cek_soal 	= $this->soal->getSoalById($id_soal);
			$total_bobot = $total_bobot + $cek_soal->bobot;
			$jawaban == $cek_soal->jawaban ? $jumlah_benar++ : $jumlah_salah++;
		}
